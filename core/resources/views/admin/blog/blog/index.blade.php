    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <a href="{{ url('admin/blog/add') }}" class="btn btn-primary float-right"><i class="fas fa-plus"></i> Add Blog</a>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
                    ...
                    <tbody>
                      @foreach ($blogs as $key => $blog)
                        <tr>
                          ...
                          @if($blog->status == null)
                          <td>
                            <a href="{{ url('admin/blog/acc/'.$blog->id) }}"><span class="badge badge-danger">No</span></a>
                          </td>
                          @else
                          <td>
                            <a href="{{ url('admin/blog/not-acc/'.$blog->id) }}"><span class="badge badge-success">Yes</span></a>
                          </td>
                          @endif

                          @if($blog->status != null)
                            @if($blog->is_featured == null)
                            <td>
                              <input type="checkbox" name="is_featured" blog-id="{{ $blog->id }}">
                            </td>
                            @else
                            <td>
                              <input type="checkbox" name="is_featured" checked="" blog-id="{{ $blog->id }}">
                            </td>
                            @endif
                          <td>
                            <a href="{{ url('admin/blog/jadi-banner/'.$blog->id) }}"><span class="badge badge-warning">Jadikan Banner</span></a>
                          </td>
                          @else
                          <td></td>
                          <td></td>
        ...
        <div class="modal-body">
          ...
          <form id="ajaxForm" class="" action="{{route('admin.blog.store')}}" method="POST">
            ...
            <div class="form-group">
              ...
              <textarea id="nicContent" class="form-control nic-edit" name="content_en" rows="8" cols="80" placeholder="Enter content"></textarea>
              <p id="errcontent" class="mb-0 text-danger em"></p>
            </div>
  ...
  $(document).ready(function(){
    ...
    $("input[type='checkbox']").click(function(e){
      ...
      if($(this).is(':checked')){
        ...
        var url = "{{ url('admin/blog/featured') }}"+'/'+id;
        window.location.href = url;

      }else{
          
        var url = "{{ url('admin/blog/not-featured') }}"+'/'+id;
        window.location.href = url;
