    ...
    <div class="container wow fadeInUp">
      <div class="row">
        ...
        <div class="section-title bottom_15 top_30">
            @if(\App::getLocale() == 'id')
            <h2 class=""><b>{{ $data->judul }}</b></h2>
            @else
            <h2 class=""><b>{{ $data->judul_en }}</b></h2>
            @endif
      ...
      <div class="row masonry clearfix">
          <!-- a work -->
          @foreach($data->photos_many as $dt)

          <a href="{{asset('gallery_images/'.$dt->photo)}}" class="col-lg-4 col-sm-6 grid-item application lightbox-icon">
            <figure class="portfolio-item">                       
                  <img style="width: 100%;height: 247px;" src="{{asset('gallery_images/'.$dt->photo)}}" alt="" />
              <figcaption>
                  <!-- <h3 class="title">{{ Str::limit($dt->judul,50) }}</h3> -->
                  <!-- <span>{{ date('d F Y',strtotime($dt->created_at)) }} </span> -->
              </figcaption> 
            </figure>
          </a> 
          @endforeach
