...
<section id="portfolio" class="portfolio graybg">
    <div class="container wow fadeInUp">
      ...
      <div class="row masonry clearfix">
          ...
          @if(\App::getLocale() == 'id')
          <a href="{{ url('gallery/'.$dt->id) }}" class="col-lg-4 col-sm-6 grid-item application ">
          @else
          <a href="{{ url('en/gallery/'.$dt->id) }}" class="col-lg-4 col-sm-6 grid-item application ">
          @endif
            <figure class="portfolio-item">                       
                  <img style="width: 100%;height: 247px;" src="{{asset('gallery_images/'.$dt->photos_one->photo)}}" alt="" />
              <figcaption>
                  @if(\App::getLocale() == 'id')
                  <h3 class="title">{{ Str::limit($dt->judul,50) }}</h3>
                  @else
                  <h3 class="title">{{ Str::limit($dt->judul_en,50) }}</h3>
                  @endif
                  <span style="font-size: 15px;">{{ date('d F Y',strtotime($dt->created_at)) }} </span>
              </figcaption> 
            </figure>
          </a> 
          @endforeach
